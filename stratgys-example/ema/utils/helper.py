...
def check_intersection(ema3, ema5, ema15, threshold=0.000001):
    """
    Check if the three EMAs are within the threshold to consider them as intersecting.
    """
...
def ema_should_open_trade(client, symbol, interval, limit, rsi_period):
    """
    Determine if a trade should be opened based on EMA intersection in the current candle.
    """
    # Fetch data
    data = fetch_ict_data(client, symbol, interval, limit=limit)
    data = data[:-1]  # Remove the last incomplete candle to ensure accuracy
    rsi = fetch_ict_ris_binance_data(data, symbol, interval, period=rsi_period, limit=limit)
    ...
    # Check for intersection in the **most recent candle only**
    ema3_prev, ema5_prev, ema15_prev = ema3.iloc[-2], ema5.iloc[-2], ema15.iloc[-2]  # Previous candle
    ema3_curr, ema5_curr, ema15_curr = ema3.iloc[-1], ema5.iloc[-1], ema15.iloc[-1]  # Current candle

    # Flags for buy/sell signals
    is_buy = False
    is_sell = False

    # Detect if the EMAs intersect **in the current candle**
    if check_intersection(ema3_curr, ema5_curr, ema15_curr):
        # Check the alignment of EMAs
        if ema3_curr > ema5_curr > ema15_curr:  # Bullish alignment
            is_buy = True

        elif ema3_curr < ema5_curr < ema15_curr:  # Bearish alignment
            
            is_sell = True

    else:
        # print(f"⏸️ لا توجد إشارة تداول على {symbol} حالياً (لا يوجد تقاطع في الشمعة الحالية).")
        return None, None

    # Ensure that the intersection **just occurred** in the current candle
    if not check_intersection(ema3_prev, ema5_prev, ema15_prev):
            # Handle buy/sell decision
        if is_buy:
            print(f"📈 إشارة شراء على {symbol} (تقاطع EMAs في الشمعة الحالية).")
            return True, "buy"
        elif is_sell:
            print(f"📉 إشارة بيع على {symbol} (تقاطع EMAs في الشمعة الحالية).")
            return True, "sell"
        # else:
        #     # print(f"⏸️ لا توجد إشارة تداول لأن التقاطع حدث في الشموع السابقة على {symbol}.")
        #     return None, None

    # No valid signal
    # print(f"⏸️ لا توجد إشارة تداول على {symbol} حالياً.")
    return None, None