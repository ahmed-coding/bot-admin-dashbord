...
class CandlestickData(models.Model):
    ...
    interval = models.CharField(max_length=10, default='1m')  # Time interval (1m, 5m, 1h, etc.)
    ...
    quote_asset_volume = models.DecimalField(max_digits=18, decimal_places=8)
    taker_buy_base_asset_volume = models.DecimalField(max_digits=18, decimal_places=8)
    taker_buy_quote_asset_volume = models.DecimalField(max_digits=18, decimal_places=8)
    number_of_trades = models.IntegerField()
    change_percentage = models.DecimalField(max_digits=5, decimal_places=2)

    class Meta:
        indexes = [
            models.Index(fields=['symbol', 'interval', 'open_time']),  # Index for fast queries
        ]
        unique_together = ('symbol', 'interval', 'open_time')  # Avoid duplicates
